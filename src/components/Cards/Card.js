  ...
  return (
    <div
      className={`transform hover:scale-110 cursor-pointer transition delay-100 lg:col-span-1 col-span-2 w-full p-4 hover:border-r-0  ${
        props.title !== "TOTAL MEMBERS" 
          ? "border-r-2"
          : props.title !== "border-r-0"
      }
      `}
    >
      <div className="flex">
      ...
      {props?.balance && <p className="text-black font-bold text-2xl">
        {props.title !== "TOTAL MEMBERS" ? `₦${props?.balance}` : props?.balance}
      </p>}
      {props?.data && <p className="text-black font-bold text-2xl">
        {props?.data}
      </p>}
      <div></div>
    </div>
