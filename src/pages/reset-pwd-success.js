import Head from "next/head";
import Image from "next/image";
import Link from "next/link";
import { IoMdCheckmarkCircleOutline } from "react-icons/io";

const SetPasswordSuccess = () => {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen  text-center">
      <Head>
        <title>Alat For Corporative</title>
        <meta name="description" content="Generated by create next app" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon-new.ico" />
      </Head>
      <main className="flex flex-col w-full flex-1">
        <div className="flex w-full justify-center relative bg-dark-purple">
          <Image
            src="/new-bg.png"
            alt="Alat for Cooperative"
            className="w-full object-fill absolute bg-dark-purple"
            width={500}
            height={500}
            priority
          />
          <div className="flex p-16 flex-col w-full text-center justify-center items-center z-10">
          <Link href="/login" className="cursor-pointer">
            <Image
              src="/logo.png"
              alt="Alat for Cooperative"
              width={75}
              height={75}
            />
            </Link>
            <div className="bg-white w-full max-w-md mt-10 p-10 py-24 rounded-md text-center">
              <div>
                <IoMdCheckmarkCircleOutline className="text-green-500 w-full text-4xl mb-4" />
                <h2 className="text-3xl font-bold mb-4 text-dark-purple">
                  Success!
                </h2>
                <p className="text-gray-500">
                  A new password has been created successfully
                </p>
              </div>
              <Link
                href="/login"
                className="rounded-md w-full px-8 py-2 text-center mt-9 inline-block font-semibold bg-dark-purple text-white hover:border hover:border-dark-purple hover:bg-white hover:text-dark-purple"
              >
                Log in to your account
              </Link>
            </div>
          </div>
        </div>
      </main>
    </div>
  );
};

export default SetPasswordSuccess;

